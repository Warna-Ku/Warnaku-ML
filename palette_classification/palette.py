...
class PaletteRGB:
    """
    .. description::
    Class representing a palette of RGB colors, each described by a TensorFlow tensor of shape (3, 1, 1).

    .. inputs::
    colors: TensorFlow tensor of shape (n_colors, 3, 1, 1).
    """
    ...
    def __init__(self, description='palette', colors=tf.zeros((1, 3, 1, 1), dtype=tf.uint8)):
        ...
        self.colors_ = tf.reshape(tf.transpose(colors, perm=[1, 2, 0]), (3, 1, -1))
    ...
    def compute_metrics_vector(self, subtone, intensity, value, contrast, thresholds=(0.5, 0.5, 0.5)):
        ...
        self.metrics_vector_ = tf.convert_to_tensor(sequence)
    ...
    def save(self, filepath='', delimiter=';'):
        ...
        if self.has_metrics_vector():
            header = header + '# metrics vector (SIVC)\n' + compact_string_(self.metrics_vector_.numpy()) + '\n'

        header = header + '# color data\n'
        np.savetxt(filename, utils.from_DHW_to_HWD(self.colors_).numpy(),
    ...
    def load(self, filename, header=False, delimiter=';'):
        ...
        if header is True:
            file = open(filename)
            file.readline()
            header_data = list(file.readline().strip())
            self.metrics_vector_ = tf.convert_to_tensor(np.array(header_data, dtype=np.uint8))
            file.close()

        self.colors_ = tf.convert_to_tensor(np.loadtxt(fname=filename, dtype=np.uint8, skiprows=int(header) * 3,
    ...
    def distance_from(self, other_palette, type='avg'):
        ...
        for color_idx in range(self.n_colors()):
            ...
            for other_color_idx in range(other_palette.n_colors()):
                ...
                other_color_hue = cv2.cvtColor((other_color_np_HWD / 255).astype(np.float32), cv2.COLOR_RGB2HSV)[0, 0, 0]
        ...
        if type == 'min':
            distance = tf.reduce_min(distances)
        elif type == 'max':
            distance = tf.reduce_max(distances)
        elif type == 'avg':

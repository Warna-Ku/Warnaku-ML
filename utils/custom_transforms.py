...
class BilateralFilter:
    """
    .. description::
    Custom transform taking a tf.Tensor as input and applying a bilateral filter (function cv2.bilateralFilter)
    to it. Returns the result as a tf.Tensor.
    """
    ...
    def __call__(self, img: tf.Tensor):
        ...
        return utils.from_HWD_to_DHW(tf.convert_to_tensor(img_filtered))
...
class PartiallyDeterministicCenterCrop:
    ...
    def __call__(self, img: tf.Tensor):
        ...
        if n > 1 - self.p:
            return tfa.image.center_crop(img, [new_height, new_width])
